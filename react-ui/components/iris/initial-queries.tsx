    ...
    try {
      ...
      const generatedQueries: Query[] = Array.isArray(data.queries)
        ? data.queries.map((q: any, i: number) => ({
            ...
            priority: 3, // All queries have the same base priority
          }))
        : []
          ...
          <div className="space-y-3">
            {queries.map((query, index) => (
              <div key={query.id} className="flex items-start gap-3 text-sm">
                <span className="text-[#999999] min-w-[20px]">{index + 1}.</span>
  ...
  return (
    <div className="animate-in fade-in duration-200">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#666666] mt-1">
              Generated based on your priority weights from clarifying questions
            </p>
          </div>
        ...
        <div className="space-y-3">
          {queries.map((query, index) => (
            <Card key={query.id} className="p-4 border-[#E5E5E5] rounded-xl hover:border-[#0A0A0A] transition-colors">
              {editingId === query.id ? (
                <div className="space-y-3">
                  <Input
                    value={editText}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1">
                    <span className="text-sm text-[#999999] min-w-[24px] mt-0.5">{index + 1}.</span>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <Button
                      ...
                      size="sm"
                      variant="ghost"
                      className="text-[#666666] hover:text-[#0A0A0A] hover:bg-[#F8F8F8] h-8 px-3 rounded-lg"
                    >
                      Edit
                    </Button>
                    <Button
                      ...
                      size="sm"
                      variant="ghost"
                      className="text-[#666666] hover:text-red-600 hover:bg-red-50 h-8 px-3 rounded-lg"
                    >
                      Delete
                    </Button>
                  </div>
        ...
        {queries.length === 0 && !loading && (
          <div className="text-center py-12 text-[#666666] bg-[#F8F8F8] rounded-xl">
            <p className="text-sm">No queries yet. They will be generated automatically from your answers.</p>
            <p className="text-xs mt-2 text-[#999999]">
              Higher priority answers will generate more focused queries.
            </p>
          </div>
        )}
